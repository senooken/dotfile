...
readonly EXCLUDE=".DS_Store .git .gitmodule"
...
if [ $is_windows ]; then
  ...
  for dotfile in "${script_dir}"/linux/*; do
    make_recursive_link "$dotfile" "$HOME/"
  done
fi
